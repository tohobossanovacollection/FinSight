...
export function StepDebts({ formData, updateFormData }: StepProps) {
  ...
  return (
    <div className="p-8 md:p-10 max-h-[60vh] overflow-y-auto custom-scrollbar">
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-white mb-2">Debts & Credit</h2>
      ...
      <div className="mb-10">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={addLoan} className="text-xs font-medium text-blue-400 hover:text-blue-300 flex items-center gap-1 bg-blue-500/10 px-3 py-1.5 rounded-full transition-colors">
            <Plus className="w-3 h-3" /> Add Loan
          </button>
        </div>
        
        {formData.loans.length === 0 ? (
          <div className="text-sm text-slate-500 italic p-4 rounded-xl border border-dashed border-white/[0.1] text-center">
            No loans added.
          </div>
        ) : (
          <div className="space-y-4">
            {formData.loans.map((loan, idx) => (
              <div key={idx} className="p-4 rounded-xl bg-white/[0.02] border border-white/[0.06] relative group">
                <button 
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-white">Credit Cards</h3>
          <button onClick={addCreditCard} className="text-xs font-medium text-blue-400 hover:text-blue-300 flex items-center gap-1 bg-blue-500/10 px-3 py-1.5 rounded-full transition-colors">
